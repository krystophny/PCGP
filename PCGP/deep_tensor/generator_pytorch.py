...
class PCGP_Builder_pytorch:
    ...
    The generated file can be imported and used in a PyTorch-based Gaussian process model.
    """
    def __init__(self):
        self.kernels = []
    ...
    def _get_body(self, kernel_object, base_kernel_arguments, symbolic_kernel, num_tasks, number_of_input_dimensions):
        ...
        for (i, j), element in np.ndenumerate(symbolic_kernel):
            ...
            if expr == 0:
                torch_expr = "torch.zeros_like(x[...,0]-y[...,0])"
    ...
    def write(self, file_name, output_dir=None):
        ...
        with open(file_path, "w") as f:
            f.write(rendered)
        print(f"Kernel written to: {file_path}")
